...
def _eval_array(indices, vals, rebase_arrays=True):
    ...
    if len(indices) > 0:
        ...
        arr = {i: _eval_val(vals.pop(0)) for i in range(len(vals))}
        ...
        if var_m:
            ...
            if not array_m:
                for key in assign.keys():
                    val = re.sub(r'\b{}\b'.format(key), str(assign[key]), val)
                array_m = _array_p.match(val)
            ...
            if array_m:
                ...
                if dim:  # explicit dimensions
                    ...
                    if any((idx.isalnum() for idx in indices)):
                        # Indices are not provided and could not be inferred
                        indices = []
                    else:
                        # Standard way, all indices are provided
